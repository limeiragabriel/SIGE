...
#================================================
from pysignfe.nf_e.manual_700.nfe_400 import NFe    as NFe_400
from pysignfe.nf_e.manual_700.nfe_400 import Det    as Det_400
from pysignfe.nf_e.manual_700.nfe_400 import autXML as autXML_400
from pysignfe.nf_e.manual_700.nfe_400 import Dup    as Dup_400
#================================================
...
class ProcessadorNotaFiscal(object):
    ...
    def montar_nota(self, nota_obj, versao='4.0'):
        #if versao == '3.10':
        #    nfe = NFe_310()
        if versao == '4.0':
            nfe = NFe_400()
    ...
    def consultar_cadastro(self, empresa, salvar_arquivos):
        ...
        if not empresa.uf_padrao:
            ...
            try:
                processo = self.nova_nfe.consultar_cadastro(cert=self.info_certificado['cert'], key=self.info_certificado['key'], cpf_cnpj=empresa.cpf_cnpj_apenas_digitos, versao=u'4.0',
    ...
    def inutilizar_notas(self, empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos):
        ...
        if not empresa.uf_padrao:
            ...
            processo = self.nova_nfe.inutilizar_faixa_numeracao(cnpj=empresa.cpf_cnpj_apenas_digitos, serie=serie, numero_inicial=numero_inicial, numero_final=numero_final, justificativa=justificativa,
                                                                cert=self.info_certificado['cert'], key=self.info_certificado['key'], versao=u'4.0', ambiente=int(ambiente), estado=empresa.uf_padrao, nfce=nfce, contingencia=False, caminho=MEDIA_ROOT)
    ...
    def consultar_nota(self, chave, ambiente, salvar_arquivos):
        ...
        processo = self.nova_nfe.consultar_nfe(chave=chave, cert=self.info_certificado['cert'], key=self.info_certificado['key'], versao=u'4.0', ambiente=int(
    ...
    def baixar_nota(self, chave, ambiente, ambiente_nacional, salvar_arquivos):
        ...
        processo = self.nova_nfe.download_notas(cnpj=cnpj, lista_chaves=[chave, ], ambiente_nacional=ambiente_nacional, cert=self.info_certificado['cert'], key=self.info_certificado['key'],
                                                versao=u'4.0', ambiente=int(ambiente), estado=uf, contingencia=False, caminho=MEDIA_ROOT)
    ...
    def efetuar_manifesto(self, chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos):
        ...
        processo = self.nova_nfe.efetuar_manifesto(cnpj=cnpj, tipo_manifesto=tipo_manifesto, chave=chave, ambiente_nacional=ambiente_nacional, cert=self.info_certificado['cert'], key=self.info_certificado['key'], versao=u'4.0',
